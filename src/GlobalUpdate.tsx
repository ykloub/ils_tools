...
const GlobalUpdate: React.FC = () => {
  ...
  const handleItemsUpdate = async (id: string) => {
    try {
      ...
      const typeId = "0f608771-d6d7-4d04-a143-f52f0b21db24";
  ...
  const fetchMTypes = async () => {
    try {
      const response = await fetch(
        "https://okapi-uae-cls01.ils.medad.com/material-types?query=cql.allRecords=1 sortby name&limit=2000",
        { headers }
      );
      if (!response.ok) {
                              ...
                              <div className="col-sm-2">
                                {item.notes
                                  .filter(
                                    (note) =>
                                      note.itemNoteTypeId ===
                                      "0f608771-d6d7-4d04-a143-f52f0b21db24"
                                  )
                                  .map((note, noteIndex) => (
                                    <input
                                      key={noteIndex}
